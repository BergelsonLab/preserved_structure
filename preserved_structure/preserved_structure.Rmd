...
Given a set of images $S_c$ containing objects belonging to a single category $c$ (e.g. cat, dog, chair), we define our prototypical vector $\hat{x}_c$ of $S_c$ as the generalized median within a representational space $U$. This is the vector with minimal sum of distances between it and all the other members of set $S_c$ in $U$. If $x$ and $y$ are vectors in space $U$, resulting from images in $S_c$ being passed through a neural network, then
...
We find that pairwise cosine distances between objects in word vector space correlate with those same pairwise distances in the image vector space. If we partition the set of inter-word distances into those that are either animate-animate, inanimate-inanimate, or mixed, we find that the pairs of distances between inanimate objects significantly correlate across our two spaces `r not_anim_corr_string`, while the other two pairings do not. 


```{r pairwise_corr, echo = F, fig.cap = cap, fig.height=5, fig.width=3.4}
...
```{r pairwise_corr_animate_vs_not, echo = F, fig.cap=cap, fig.width=3.4}
...
```{r animacy_aoa, echo=F, fig.height=4, fig.width=3.1}
...
# qplot(wb_comp_long$mean_prop_comp)
