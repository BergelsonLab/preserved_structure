...
# Discussion
The results above revealed a significant correspondence between representations learned by two different algorithms operating over inputs in two fundamentally different encoding formats (i.e. visual and linguistic). We find that not only are the relative distances among these 27 common, early-learned items correlated across word- and image-space, but that even at the item-level, the closest words (i.e. neighbors) in both spaces overlap as well. Moreover, words corresponding to items with more neighbors were reportedly less well known by young children. Notably, the only immediate common ground between these image- and word-based representations are the real life concepts they both aim to model. This is particularly noteworthy given the enormous dimensionality of our feature spaces, and the fact that these algorithms were placed under no pressure to find homologous representations. This is not to imply that the representations learned by these two algorithms are plausible cognitive mechanisms for computing similarity between objects, or that children are employing similar processes (although CNN architectures were originally inspired by primary visual cortex [@fukushima1982neocognitron] and GloVe has recently found application in decoding semantic representations from brain activity [@pereira2018toward]). Our current results are presented primarily to demonstrate that there is indeed conserved structure inherent *in the data* learners are presented with, and any account of early learning that treats linguistic and visual information as purely separable must account for this. The behavioral effect we report lends further weight to the notion that infants may be sensitive to this regularity across modalities, exhibiting degraded performance in their word acquisition when this cue is overloaded.

The notion that we can make inferences about one aspect of an object given another aspect is not surprising or controversial. Rather than considering multiple dimensions of a word or concept at once, as real learners must, here we show that even with the experiences parcelled out separately into visual and linguistic spaces, what counts as 'similar' is conserved to some degree. In future iterations of this work we hope to train image vectors on decontextualized images using unsupervised algorithms, in hopes of studying a more clean signal of the visual characteristics inherent to the object at hand. This would avoid residuals that might be incorporated as a result of the contexts objects are situated in or the category structure imposed during supervised training. 
